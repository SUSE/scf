...
Name      | Effect |
--------------- | ---- |
`run`      | Set up SCF on the current node (`bin/run.sh`) |
...
### How do I run smoke and acceptance tests?
  ...
  On the Vagrant box, when `scf-status` reports all roles are running, enable `diego_docker` support with
  ...
  ```bash
  run-role.sh /home/vagrant/scf/bin/settings/ smoke-tests
  run-role.sh /home/vagrant/scf/bin/settings/ acceptance-tests-brain
  run-role.sh /home/vagrant/scf/bin/settings/ acceptance-tests
  run-role.sh /home/vagrant/scf/bin/settings/ acceptance-tests-flight-recorder
  ```
...
#### How do I run a subset of SCF acceptance tests?
  ...
  ```bash
  run-role.sh /home/vagrant/scf/bin/settings/ acceptance-tests-brain --env INCLUDE=pattern --env EXCLUDE=pattern
  ```

  For example to run just `005_sso_test.sh` and `014_sso_authenticated_passthrough_test.sh`:
  ```bash
  run-role.sh /home/vagrant/scf/bin/settings/ acceptance-tests-brain --env INCLUDE=sso
  ```
  ...
  ```bash
  run-role.sh /home/vagrant/scf/bin/settings/ acceptance-tests-brain --env 'EXCLUDE=\b\d{3}_' -v /tmp/tests:/tests
  ```

  Or explicitly select only the mounted tests with:
  ```bash
  run-role.sh /home/vagrant/scf/bin/settings/ acceptance-tests-brain --env 'INCLUDE=^/tests/' -v /tmp/tests:/tests
  ```
...
#### How do I run a subset of Cloud Foundry acceptance tests?
  ...
  ```bash
  run-role.sh /home/vagrant/scf/bin/settings/ acceptance-tests --env CATS_SUITES=-suite,+suite
  ```
