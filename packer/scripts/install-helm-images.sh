...
wget -O /tmp/txtplate https://github.com/aarondl/txtplate/releases/download/v0.0.3/txtplate-linux-amd64
...
get_helm_images() {
    ...
    for i in $(find "${DIR}" -type f -name Chart.yaml); do
        ...
        grep --fixed-string --no-filename 'image:' ${DIR}/templates/* \
            | sed 's@\.Values@@g' \
            | /tmp/txtplate "${VALUES:-${DIR}/values.yaml}" \
...
get_helm_images /home/scf/console
get_helm_images /home/scf/console/charts/mariadb
