...
local_ip=`${ROOT}/container-host-files/opt/hcf/bin/get_ip`
...
(
  if [ ! -f $ca_path/intermediate/private/${certs_prefix}-root.chain.pem ] ; then
    ...
    cp ${ROOT}/container-host-files/opt/hcf/bin/cert/intermediate_openssl.cnf ./
    cp ${ROOT}/container-host-files/opt/hcf/bin/cert/root_openssl.cnf ./

    # generate ha_proxy certs
    bash ${ROOT}/container-host-files/opt/hcf/bin/cert/generate_root.sh
    bash ${ROOT}/container-host-files/opt/hcf/bin/cert/generate_intermediate.sh
    bash ${ROOT}/container-host-files/opt/hcf/bin/cert/generate_host.sh "${certs_prefix}-root" "*.${domain}"
