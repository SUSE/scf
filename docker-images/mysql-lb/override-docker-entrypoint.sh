...
if [[ -n "${MYSQL_PROXY_HOST}" ]] ; then
    echo "  server ${MYSQL_PROXY_HOST}.hcf:3306 check port 1936" >> $CFG
fi
if [[ -n "${MYSQL_PROXY_HA_HOST_1}" ]] ; then
    echo "  server ${MYSQL_PROXY_HA_HOST_1}.hcf:3306 backup check port 1936" >> $CFG
fi
if [[ -n "${MYSQL_PROXY_HA_HOST_2}" ]] ; then
    echo "  server ${MYSQL_PROXY_HA_HOST_2}.hcf:3306 backup check port 1936" >> $CFG
fi
