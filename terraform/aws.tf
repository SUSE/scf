...
output "floating_domain" {
    value = "${aws_instance.core.public_ip}.nip.io"
}
...
resource "null_resource" "DOMAIN" {
    triggers = {
        DOMAIN = "${aws_instance.core.public_ip}.nip.io"
    }
}
...
resource "aws_security_group" "frontend" {
    ...
    name        = "${var.cluster-prefix}-frontend"
    description = "Frontend: http/s, ssh, loggregator, ..."
    vpc_id      = "${aws_vpc.cluster.id}"
    ...
    #tcp   80 - http
    #tcp  443 - https
    #tcp 2222 - diego-access
    #tcp 4443 - loggregator
    #udp   53 - windows-dns
...
resource "aws_instance" "core" {
    ...
    provisioner "remote-exec" {
        inline = [
            "echo 127.0.0.1 ip-$(echo ${aws_instance.core.private_ip} | tr . -) | sudo tee -a /etc/hosts",
...
resource "null_resource" "core_final_setup" {
    triggers = {
        core_ip = "${aws_instance.core.public_ip}"
    }
