function GeneratePassword { [CmdletBinding()] param([int]$BytesOfEntropy = 32)
...
function RunProcessAsTask {
  ...
  $regs = Register-ScheduledJob -Name $id -ScriptBlock $invokeTask -ArgumentList $id, $FilePath, $Arguments, $envVars -ScheduledJobOption (New-ScheduledJobOption -RunElevated) -Credential $idCredentials
  ...
  # Wait for exit message to break the loop and redirect the output from the pipe to stdout
  while (($line = $sr.ReadLine()) -ne $null)
  {
    $mes = ($line | ConvertFrom-Json)
    if ($mes.'type' -eq 'exit') {
      echo ( "Exit code from `"$FilePath`": " + $mes.'value' )
      $exitCode = $mes.'value'
      break
...
Export-ModuleMember GeneratePassword, RunProcessAsTask
